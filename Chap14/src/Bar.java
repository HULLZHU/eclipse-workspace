...
public class Bar extends FlowPane{
    ...
    private Text[] getDescriptions() {
		for ( int i= 0; i < descriptions.length ; i ++  ) {
			double temp = (Math.round(percentiles[i]*100))/100.0;
	...
	private void printBars() {
		...
		for ( int i = 0 ; i < percentiles.length; i++) {
			...
			rectangles[i].setFill(Color.color(Math.random(), Math.random(), Math.random()));
			pane.setBottom(rectangles[i]);
			pane.setTop(descriptions[i]);
			pane.setAlignment(descriptions[i],Pos.BOTTOM_CENTER);
