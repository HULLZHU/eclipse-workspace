...
public class PieChart extends Pane {
	...
	public void printPieChart() {
		...
		for ( int i = 0; i < arcs.length ; i ++  ) {
			double positionRadians = Math.toRadians(-arcs[i].getStartAngle()-0.5*arcs[i].getLength());
			descriptions[i].xProperty().bind(arcs[i].centerXProperty().add(arcs[i].radiusXProperty().multiply(Math.cos(positionRadians)).multiply(0.8)));
			descriptions[i].yProperty().bind(arcs[i].centerYProperty().add(arcs[i].radiusYProperty().multiply(Math.sin(positionRadians)).multiply(0.8)));
